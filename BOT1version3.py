        ...
        while True:
            ...
            for elemento in elementos:
                ...
                if datetime_str not in fechas_usadas:
                    ...
                    # Es lo más optimo para encontrar el maximo numero de publicaciones dentro de las ultimas 24 horas
                    if contando_fechas == 18:
                    ...
                    if last_24_hours <= datetime_publicacion <= now:
                        ...
                        # Aumentamos el contador si la publicacion no esta dentro de las ultimas 24 horas
                        contando_fechas = contando_fechas +1

            if salir == True:
         
                break
            else:
                # Pongo este try-except por si ya ha dado click a todos los botones posibles y no hay mas noticias
                try:
        ...
        for link in links:
            ...
            try:
                ...
                if 'graphics' in sb.get_current_url():
                    continue
                # Para comprobar que no haya ningun captcha
                while True:
                    try:
                        ...
                        sb.uc_open_with_reconnect(link,10)
...
def capitalbrief(headers):
    ...
    with requests.Session() as session:
        ...
        for link in links:
            ...
            respuesta = session.get(link)
            soup = BeautifulSoup(respuesta.text,'lxml')
            titulo = soup.find('h1',class_=re.compile('^sm:')).get_text(strip=True)
            cuerpo_4 = ""
