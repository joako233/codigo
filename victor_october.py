        ...
        while True:
            ...
            for elemento in elementos:
                ...
                if datetime_str not in fechas_usadas:
                    ...
                    if (now - datetime_publicacion).days >= 3:
        ...
        for link in links:
            if link not in visitadas:
                ...
                if convertido >= operacion:
                    ...
                    if len(parrafos) == 0:
                        print(f"Aplicando segundo en {link}")
                        parrafos = cuerpo.find_all('p',class_='dcr-an2vbh')
                    if len(parrafos) == 0:
                        print(f"Aplicando tercero en {link}")
        ...
        for link in links:
            ...
            if titulo is None:
                ...
                for p in parrafos:
                    ...
                    for letra in buscar:
                        if p.find(f'{letra}'):
                            reemplazo = p.find(f'{letra}').get_text(strip=True)
                            if reemplazo +"." in texto:
                                texto = texto.replace(reemplazo," "+reemplazo)
                            else:
                                texto = texto.replace(reemplazo," "+reemplazo+" ")
                ...
                for p in parrafos:
                    ...
                    for letra in buscar:
                        if p.find(f'{letra}'):
                            reemplazo = p.find(f'{letra}').get_text(strip=True)
                            if reemplazo +"." in texto:
                                texto = texto.replace(reemplazo," "+reemplazo).strip()
                            else:
                                texto = texto.replace(reemplazo," "+reemplazo+" ").strip()
